"""v0.0.1 compatibility surface — these aliases are kept forever."""

import math

import pytest

from shewhart import beyond_limits, constants, imr_limits


def test_imr_limits_hand_computed():
    # x = [10, 12, 11, 13, 12]: MRs = [2, 1, 2, 1] -> MRbar = 1.5, mean = 11.6
    lim = imr_limits([10, 12, 11, 13, 12])
    sigma = 1.5 / constants.d2(2)
    ...
    assert math.isclose(lim["mr_ucl"], constants.D4(2) * 1.5, rel_tol=1e-12)
...
def test_beyond_limits_flags_outliers():
    lim = imr_limits([10, 12, 11, 13, 12])
    assert beyond_limits([11.0, 100.0, -50.0], lim) == [False, True, True]
